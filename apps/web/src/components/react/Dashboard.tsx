...
export default function Dashboard({ locale }: Props) {
  const tr = locale === 'tr'
  const { authenticated, loading, login, logout, token, username } = useAuth()
  ...
  if (!authenticated) {
    return (
      <div className="mx-auto max-w-lg rounded-2xl border border-green-900/30 bg-green-950/20 p-8 text-center">
        ...
        <button
          onClick={login}
          className="mt-6 inline-flex items-center gap-2 rounded-lg bg-green-900/30 px-6 py-3 text-sm font-medium text-green-400 transition hover:bg-green-900/30"
        >
  ...
  return (
    <div className="space-y-8">
      {/* User card */}
      <div className="flex items-center justify-between rounded-xl border border-green-900/30 bg-green-950/20 p-5">
        <div className="flex items-center gap-4">
          {username ? (
            <img
              src={`https://github.com/${username}.png?size=80`}
              alt={username}
              className="h-10 w-10 rounded-full border border-green-900/50"
            />
          ) : (
            <div className="flex h-10 w-10 items-center justify-center rounded-full border border-green-900/50 bg-green-950/50 text-green-600">
              <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
          ...
          <div>
            <p className="text-sm font-semibold text-green-400">
              {username ?? (tr ? 'Kullanıcı' : 'User')}
            </p>
            <p className="text-xs text-green-700">
              {tr ? 'GitHub ile giriş yapıldı' : 'Signed in with GitHub'}
            </p>
          </div>
        </div>
        <div className="flex gap-2">
          <button
            onClick={exportData}
            disabled={exporting}
            className="rounded-lg border border-green-900/30 px-3 py-1.5 text-xs text-green-600 transition hover:border-green-500/50"
          >
      ...
      {typingSummary && typingSummary.recent.length > 2 && (
        <div className="rounded-xl border border-green-900/30 bg-green-950/20 p-6">
          <h3 className="mb-4 text-sm font-medium text-green-600">
            ...
            {typingSummary.recent.slice().reverse().map((s, i) => {
              ...
              return (
                <div key={i} className="group relative flex-1">
                  <div
                    className="w-full rounded-t bg-green-500/60 transition-colors hover:bg-green-500"
                    style={{ height: `${Math.max((s.wpm / maxWpm) * 100, 4)}%` }}
                  />
                  <div className="absolute bottom-full left-1/2 mb-1 hidden -translate-x-1/2 rounded bg-green-900/30 px-2 py-1 text-xs text-green-400 group-hover:block">
                    {s.wpm} WPM
                  </div>
                </div>
              )
        ...
        <div className="rounded-xl border border-green-900/30 bg-green-950/20 p-6">
          ...
          <div className="mt-4 space-y-2">
            {lessonProgress.map((l) => (
              <div key={l.lesson_id} className="flex items-center justify-between text-xs text-green-600">
                <span className="flex items-center gap-2">
                  {l.completed ? <span className="text-green-400">✓</span> : <span className="text-green-800">○</span>}
                  {l.lesson_id}
                </span>
        ...
        <div className="rounded-xl border border-green-900/30 bg-green-950/20 p-6">
          ...
          <div className="space-y-2">
            {vimStats.slice(0, 10).map((s, i) => (
              <div key={i} className="flex items-center justify-between text-xs text-green-600">
                <span className="font-medium text-green-400">{s.game}</span>
                <span className="font-mono">
      ...
      {(!typingSummary || typingSummary.totalTests === 0) && vimStats.length === 0 && (
        <div className="rounded-xl border border-green-900/30 bg-green-950/20 p-12 text-center">
